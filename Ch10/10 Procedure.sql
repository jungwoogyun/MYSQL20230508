...
select * from usertbl;
-- usertbl에서 특정열이름을 in으로 받아  min과 avg를 out하는 프로시저를 만드세요
drop procedure proc_getMinAvgValue;
delimiter $$
create procedure proc_getMinAvgValue(in col char(100), out minval char(100),out avrval char(100) )
begin
    select 
    case col 
...
set @minv='';
set @avgv='';
call proc_getMinAvgValue('birthyear',@minv,@avgv);
select @minv;
select @avgv;
